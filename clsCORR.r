...
stcc <- setClass("stcc_class",contains = c("namedList"))
...
setGeneric(name="stcc",def=function(Tobs,st,type,p1,p2,trpars,shape,speedopt,...){
  standardGeneric("stcc")
...
setMethod(f="stcc",signature = sig1,
...
setMethod(f="stcc",signature = sig2,
...
EstimateSTCC <- function(z,stccObj,calcHess=FALSE,verbose=FALSE) {
  ...
  if (tmp$convergence==0) {
    ...
    if (calcHess) {
      ...
      try(STCC$Estimated$stderr <- sqrt(-diag(qr.solve(tmp$hessian))))
    }
