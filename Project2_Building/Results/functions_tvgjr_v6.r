...
setMethod(f="newGARCH",signature = sig1,
          definition = function(type,pars, ...){
            # Validate params
            if (is.null(type)) stop("Error: missing 'type'\nAll parameters are required to construct a GARCH object")
            if (is.null(pars)) stop("Error: missing 'pars'\nAll parameters are required to construct a GARCH object")
            #
...
dg.dt <- function(tv){
  ...
  if (tv$shape[1]==TRshape$none) rtn <- matrix(nrow=tv$Tobs,ncol=1) else rtn <- matrix(nrow=tv$Tobs,ncol=numTVpars+1)
...
EstimateGARCH <- function(w,garch,calcHess=FALSE,verbose=FALSE) {
  ...
  if (GARCH$type == GARCHtype$constant) {
    ...
    #else call optim() to get the estimate
    if (verbose) GARCH$optimcontrol$trace <- 10
...
EstimateM_TVGARCH <- function(e,ntv,ngarch,stcc,focus,optimcontrols=NULL,var_target=TRUE){
  ###  Inputs:  ###
  # e
  # ntv
  # ngarch
  ...
  if (focus=="TV") {
    ...
    if (!is.null(optimcontrols)) TVoptimcontrol <- optimcontrols$TV else TVoptimcontrol <- list(fnscale = -1,maxit=1000,reltol = 1e-6)
    ## Call optim
  ...
  if (focus=="GARCH") {
    ...
    # Set the optim controls
    if (!is.null(optimcontrols)) GARCHoptimcontrol <- optimcontrols$GARCH else GARCHoptimcontrol <- list(fnscale = -1,maxit=1000,reltol = 1e-6)
    ## Call optim
    tmp = optim(optimpars,myLogLik.multivar.TVGARCHSTCC,e,ntv,ngarch,stcc,focus,var_target,return_ll=TRUE,
  ...
  if (focus=="STCC") {
    ...
    # Set the optim controls
    if (!is.null(optimcontrols)) STCCoptimcontrol <- optimcontrols$STCC else STCCoptimcontrol <- list(fnscale = -1,maxit=1000,reltol = 1e-5)
    ## Call optim
    tmp = optim(optimpars,myLogLik.multivar.TVGARCHSTCC,e,ntv,ngarch,stcc,focus,var_target,return_ll=TRUE,
                    gr=NULL,method="BFGS",control=STCCoptimcontrol)
    
    # # Test:
    # ll <- myLogLik.multivar.TVGARCHSTCC(optimpars,e,ntv,ngarch,stcc,focus,var_target,return_ll = TRUE)
