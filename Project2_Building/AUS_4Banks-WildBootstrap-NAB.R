...
G2 <- newGARCH(GARCHtype$general, c(0.02,0.05,0.95))
e <- GenerateRefData_WithGarch(G,42,1,3000,"deleteme.rds")
plot(e,type="l")
...
for (i in seq_along(randDraw)) {
  e_b <- c(e_b,e[(i*B_len+1):(i*B_len+B_len)])
}
plot(e_b,type="l")

g1 <- EstimateGARCH(e,G2)
g2 <- EstimateGARCH(e_b[1:2700],G1)

# End: Construct Block vector

n_t <- n_t[1:3000]
e1 <- e <- g_data[,1]
TV$Estimated$condvars <- var(e)
TEST1 <- myTest.TV.noGARCH.TR2(e1,TV)
e2 <- n_t*(e/sqrt(var(e)))
TV$Estimated$condvars <- var(e2)
TEST2 <- myTest.TV.noGARCH.TR2(e2,TV)
TV$Estimated$condvars <- 1
TEST3 <- myTest.TV.noGARCH.TR2(e2,TV)
